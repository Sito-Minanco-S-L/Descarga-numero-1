...
def mostrar_grafica_regresion(modelo, X, y):
    ...
    layout = [
        ...
        [sg.Button('OK', size=(10, 2), pad=((20, 0), 3), button_color=('white', 'green')),sg.SaveAs('Guardar Modelo', size=(10, 2), pad=((20, 0), 3), button_color=('white', 'blue'),file_types=(('FLP', '.flp'),),default_extension=str('.farlopa'),key='--FILE--' )]]
...
def interface(dfs:dict):
    ...
    layout1 = [
            ...
            [sg.Text(key='--PRUEBA--')],
            [sg.Text('Selecciona un archivo', key='-ho-')]
            [sg.Text('Selecciona un archivo', key='-la-')]
            [sg.AddMenuItem(key)]
        ]
    ...
    while True:
        ...
        if event == 'Cargar Archivo':
            ...
            window['-ho-'].update(values[sg.Text('Guardar Modelo de Regresión Lineal')])
            window['-la-'].update(values[sg.FileSaveAs(key='fig_save',file_types=(('txt', '.txt'),))])
    ...
    # Crear ventana menu
    window = sg.Window('Aplicación de Regresión', layout1, finalize=True, size = (900,500))
