    ...
    while True:
        ...
        if event == 'Realizar Regresión Lineal':
            ...
            if x and y:
                ...
                formula = f"F(x) = {coeficientes[0]:.2f}"  # Término de la constante

    # Agregar los términos para las variables predictoras
                for i, coef in enumerate(coeficientes[1:], start=1):
                    formula += f" {'+' if coef >= 0 else '-'} {abs(coef):.2f} ({nombres_columnas[i-1]})"
