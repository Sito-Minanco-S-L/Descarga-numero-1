...
def interface(dfs:dict):
    ...
    layout = [
    [sg.InputText(default_text= 'Selecciona un archivo: ', key='-Archivo-', disabled=True, ), sg.FileBrowse(file_types=(("All Files", "*.*"),))],
    [col1],
    [col2],
    [sg.Table(values=[], headings=[], display_row_numbers=True, auto_size_columns=True, key='-TABLE-')],
    [sg.Frame('',[],key='---PRUEBA---')],
    #[sg.Multiline(size=(150, 20), key='-TABLE-', auto_refresh=True, visible = False, default_text = '')],
    
    #[sg.Table(values=[], headings=[], display_row_numbers=True, auto_size_columns=True, key='-TABLE-')],
    #[sg.Multiline(size=(80, 20), key='-OUTPUT-', autoscroll=True, font=('Courier New', 10))],
    [sg.Frame('',[[sg.Button('Cargar Archivo', size=(20, 2)),
        sg.Button('Mostrar tabla de datos', size=(20, 2), button_color=('black', 'yellow')),
    ...
    while True:
        ...
        if event == 'Cargar Archivo':
            ...
            try:
                ...
                #window['-TABLE-'].update(values=[table_headings])
                window.extend_layout(window['---PRUEBA---'], [[sg.Table(table_data, table_headings)]])


                #window['-TABLE-'].update(value=dfs[selected_file].to_string(index=False), visible = True)
                #window['-TABLE-'].update(values=df.values.tolist(), headings=df.columns.tolist())
                #pg.show(dfs[selected_file])
