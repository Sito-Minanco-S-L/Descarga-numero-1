...
def make_table(num_rows, num_cols):
    ...
    data[0] = [word() for _ in range(num_cols)]
    for i in range(0, num_rows):
        data[i] = [i, word(), *[number() for _ in range(num_cols - 1)]]
    return data
...
def main_example1():
    ...
    edit = False
    # ------ Make the Table Data ------
    data = make_table(num_rows=1_000, num_cols=7)
    headings = [f'Col {col}' for col in range(len(data[0]))]
    sg.set_options(dpi_awareness=True)
    ...
    # Get DataFrame values as a list of lists
...
def main_example1():
    ...
    # Convert data to DataFrame
    dfs = pd.read_excel('housing.xlsx')
    df = dfs.select_dtypes(include=[np.number])

    # Use DataFrame columns as headings
